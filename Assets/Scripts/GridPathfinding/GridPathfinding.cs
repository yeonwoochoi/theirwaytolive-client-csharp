...
namespace GridPathfinding {
    ...
    public class GridPathfinding {
        ...
        public void RaycastWalkable(int unWalkableLayerMask) {
            for (int i = 0; i < widthMax; i++) {
                for (int j = 0; j < heightMax; j++) {
                    Vector3 nodeWorldPosition = mapNodes[i][j].GetWorldVector(worldOrigin, nodeSize);
                    RaycastHit2D raycastHit = Physics2D.Raycast(nodeWorldPosition, Vector2.zero, 0f, unWalkableLayerMask);
        ...
        public void PrintMap(Transform prefabWalkable, Transform prefabUnwalkable) {
            for (int x = 0; x < mapNodes.Length; x++) {
                for (int y = 0; y < mapNodes[x].Length; y++) {
                    ...
                    UnityEngine.Object.Instantiate(pathNode.IsWalkable() ? prefabWalkable : prefabUnwalkable, worldOrigin + new Vector3(x * nodeSize, y * nodeSize), Quaternion.identity);
                }
            }
        }
